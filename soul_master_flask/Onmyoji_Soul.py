...
class Soul:
    ...
    def __str__(self):
        return f'This is level {self.level} {self.name} of slot {self.slot}.'


def boost(s: Soul, exp):
    global non_prime_attrs
...
if __name__ == "__main__":
    ...
    print(
        f"Count: {cnt}, Average:{sum(valid_results) / cnt:.2f}, Max:{max(valid_results)}, Time: {time.time() - start_time:.2f} seconds")
